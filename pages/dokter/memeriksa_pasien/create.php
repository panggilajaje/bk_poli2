...
<script>
    ...
    $('#id_obat').select2(); // Menginisialisasi Select2
    $('#id_obat').on('change', function() {
        var selectedValuesArray = $(this).val();
        var selectedList = $('#selected_obat_list');
        selectedList.empty(); // Kosongkan daftar sebelumnya

        // Kalkulasi Total Harga
        var totalHarga = 150000; // Biaya periksa tetap
        if (selectedValuesArray) {
            selectedValuesArray.forEach(function(value) {
                var parts = value.split("|");
                if (parts.length === 2) {
                    var id = parts[0];
                    var harga = parseFloat(parts[1]);
                    totalHarga += harga;

                    // Tambahkan ke daftar pratinjau
                    selectedList.append('<li>Obat ID: ' + id + ' - Harga: Rp.' + harga + '</li>');
                }
            });
        }
